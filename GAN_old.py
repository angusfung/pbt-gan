...
class GAN(object):
    ...
    def train(self):
        ...
        if could_load:
            ...
            print(" [*] Load SUCCESS")
            print(" [*] Loading checkpoint from epoch {} and batch id {}".format(start_epoch, start_batch_id))
        else:
            start_epoch = 0
            start_batch_id = 0
            counter = 1
                ...
                if np.mod(counter, 300) == 0:
                    ...
                    save_images(samples[:manifold_h * manifold_w, :, :, :], [manifold_h, manifold_w],
                                './' + check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_train_{:02d}_{:04d}.png'.format(
                                    epoch, idx))
                    save_images_2(samples[:manifold_h * manifold_w, :, :, :], [manifold_h, manifold_w],
                                './' + check_folder(self.result_dir + '/' + self.model_dir) + '/' + 'image_2' + '/'+ self.model_name + '_train_{:02d}_{:04d}.png'.format(
                                    epoch, idx))

                    save_matplot_img(samples[:manifold_h * manifold_w, :, :, :], [manifold_h, manifold_w],
                                './' + check_folder(self.result_dir + '/' + self.model_dir) + '/' + 'image_3' + '/'+ self.model_name + '_train_{:02d}_{:04d}.png'.format(
                                    epoch, idx))
    ...
    def visualize_results(self, epoch):
        ...
        save_images_2(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + 'image_2' + '/' + self.model_name + '_epoch%03d' % epoch + '_test_all_classes.png')
        save_matplot_img(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + 'image_3' + '/' + self.model_name + '_epoch%03d' % epoch + '_test_all_classes.png')
